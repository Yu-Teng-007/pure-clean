...
- **清理工具**：汇总各场景清理（安全 / 开发 / 系统 / 大文件 / 重复 / 闲置 / 安装包 / Docker / 系统进阶）
...
- **系统进阶**：浏览器隐私数据（Cookie / 历史 / 登录态，默认不选）、下载器与游戏启动器缓存、卸载残留启发式扫描；WinSxS / `hiberfil.sys` / `pagefile.sys` 仅检测并给出建议，WinSxS 可一键打开 Windows 磁盘清理
